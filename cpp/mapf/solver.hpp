...
public:
    ...
    MinimumSolver(Graph *G,Config starts,Config goals,int max_comp_time=120000){
        this->G=G;
    ...
    void setMaxTimestep(int timestep){ max_timestep=timestep; P->setMaxTimestep(timestep);}
...
public:
    Solver(Problem *_P);
    Solver(Graph *G,Config starts,Config goals,int max_comp_time=60000);
    virtual ~Solver();
    ...
    void resetEnv(Nodes starts,Nodes goals,Grid *graph=nullptr);

    //return the shortest path of agent within the fov
    std::vector<std::vector<int>> localShortestPathInFOV(int agent,int fov_radius);

    //heuristic map
    std::vector<std::vector<int>> getHeuristicMap(int agent,int fov_radius);
};
