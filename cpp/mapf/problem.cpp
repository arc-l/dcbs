    ...
    : instance(_instance), instance_initialized(true)
{
    ...
    if (!file)
        halt("file " + instance + " is not found.");
...
Problem::Problem(Graph *g, Config _config_s, Config _config_g,
                int _max_comp_time, int _max_timestep)
    : G(g),
    config_s(_config_s),
    config_g(_config_g),
    num_agents(_config_g.size()),
    max_timestep(_max_timestep),
    max_comp_time(_max_comp_time),
    instance_initialized(true)
{
    std::random_device dev;
    MT = new std::mt19937(dev());
...
{
    // std::cout<<"I am debuggging "<<config_s.size()<<std::endl;
...
void Problem::setRandomStartsGoals()
{
    ...
    while (true)
    {
        while (G->getNode(starts[i]) == nullptr)
        {
            ++i;
            if (i >= N)
                halt("number of agents is too large.");
        }
        config_s.push_back(G->getNode(starts[i]));
        if ((int)config_s.size() == num_agents)
            break;
        ++i;
    }
    ...
    while (true)
    {
        while (G->getNode(goals[j]) == nullptr)
        {
            ++j;
            if (j >= N)
                halt("set goal, number of agents is too large.");
        }
        // retry
        ...
        if ((int)config_g.size() == num_agents)
            break;
        ++j;
    }
}
    ...
    while ((int)config_g.size() < getNum())
    {
        while (true)
        {
            ...
            if (!path.empty())
            {
                ...
                for (auto v : path)
                {
                    if (!inArray(v, prohibited))
                        prohibited.push_back(v);
                }
                break;
            }
        }
