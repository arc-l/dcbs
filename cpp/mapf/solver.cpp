...
void Solver::resetEnv(Nodes starts,Nodes goals,Grid*graph){
    if(graph!=nullptr){
        std::cout<<"delete G"<<std::endl;
        delete this->G;
        std::cout<<"deleted G"<<std::endl;
        this->distance_table=DistanceTable(starts.size(),std::vector<int>(G->getNodesSize(),max_timestep));
        this->P->setG(graph);
    }
...
std::vector<std::vector<int>>Solver::localShortestPathInFOV(int agent,int fov_radius){
    std::vector<std::vector<int>> binary_map(2*fov_radius+1,std::vector<int>(2*fov_radius+1,0));
    auto si=P->getStart(agent);
    Path full_shortest_path=getPath(si,P->getGoal(agent));
    int x0=si->pos.x-fov_radius;
    int y0=si->pos.y-fov_radius;
    for(auto node:full_shortest_path){
        if(node->manhattanDist(si)>2*fov_radius){
            break;
        }
        int i=node->pos.x-x0;
        int j=node->pos.y-y0;
...
std::vector<std::vector<int>>Solver::getHeuristicMap(int agent,int fov_radius){
    std::vector<std::vector<int>> binary_map(2*fov_radius+1,std::vector<int>(2*fov_radius+1,0));
    int curr_dist=pathDist(agent);
    auto si=P->getStart(agent);
    int x0=si->pos.x-fov_radius;
    int y0=si->pos.y-fov_radius;
    for(int i=0;i<2*fov_radius+1;i++){
        for(int j=0;j<2*fov_radius+1;j++){
            
            auto node=G->getNode(i+x0,y0+j);
            if(node==nullptr) continue;
            if(pathDist(agent,node)<curr_dist) binary_map[i][j]=1;
        }
    }
    return binary_map;
}