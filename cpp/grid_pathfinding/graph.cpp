...
Grid::Grid(int xmax,int ymax,double obstacle_density):
    Grid(xmax,ymax,std::vector<std::vector<int>>()){
    ...
    // Find a valid start vertex
        ...
        if (nullptr) return std::vector<Node*>();
...
Node *Grid::getNode(int x, int y) const {
    if(x<0 or y<0 or x>=width or y>=height) return nullptr; 
...
std::vector<Pos> Grid::getObstacles(){
    ...
    for(int x=0;x<width;x++){
        for(int y=0;y<height;y++){
            if(existNode(x,y)==false) obstacles.push_back(Pos(x,y));
        }
    }
    return obstacles;
}
...
std::vector<std::vector<int>> Grid::getBinaryMap(){
    ...
    for(int x=0;x<width;x++){
        for(int y=0;y<height;y++){
            if(existNode(x,y)==false) binary_map[x][y]=1;
        }
    }
    return binary_map;
...
bool Grid::is_blocked(int x,int y) const{
    if(existNode(x,y)==true) return false;
    return true;
}
